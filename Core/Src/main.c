...
int main(void)
{
  ...
  MX_USART3_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Receive_IT(&huart3, &uartRxByte, sizeof(uint8_t));
  /* USER CODE END 2 */
...
/**
  * @brief  Overwrite the standard printf function to output on USART-STLINK
  * @param  ch character
...
{
  HAL_UART_Transmit(&huart3, (uint8_t *)&ch, sizeof(uint8_t), HAL_MAX_DELAY);
  return ch;
}
/* USER CODE END 4 */
